...
impl Client {
    ...
    async fn parse_json<T: DeserializeOwned>(&self, rb: Result<reqwest::RequestBuilder, reqwest::Error>) -> Result<T, ApiError> {
        match rb {
            Ok(rb) => match rb.send().await {
                Ok(res) => match res.status() {
                    reqwest::StatusCode::OK => Ok(res.json().await.expect("Unexpected json response from the API, cannot parse json")),
